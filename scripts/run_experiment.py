...
For each (demo_count, seed) pair this script:
  ...
  3. Runs SRPO from the SFT checkpoint
...
def main(
    ...
    for n_demos in demo_counts:
        for seed in seeds:
            ...
            log.info(f"\n{'='*60}\n  SFT  {tag}\n{'='*60}")
            rc = _run([
                sys.executable, str(PROJECT_ROOT / "scripts" / "train_sft.py"),
                "--data", data_path,
                "--num-demos", str(n_demos),
                "--checkpoint", checkpoint,
                "--lr", str(sft_cfg["lr"]),
                "--batch-size", str(sft_cfg["batch_size"]),
                "--epochs", str(sft_cfg["num_epochs"]),
                "--eval-every", str(sft_cfg["eval_every"]),
                "--eval-episodes", str(sft_cfg["eval_episodes"]),
                "--max-steps", str(sft_cfg["max_steps"]),
                "--seed", str(seed),
                "--env", env_id,
                wandb_flag,
            ])
            ...
            for mode, rl_cfg in [("sparse_rl", sparse_cfg), ("srpo", srpo_cfg)]:
                rl_save = str(PROJECT_ROOT / "checkpoints" / mode / tag)
                log.info(f"\n{'='*60}\n  {mode.upper()}  {tag}\n{'='*60}")
                cmd = [
                    sys.executable, str(PROJECT_ROOT / "scripts" / "train_srpo.py"),
                    "--sft-checkpoint", sft_best,
                    "--checkpoint", checkpoint,
                    "--action-dim", str(action_dim),
                    "--mode", mode,
                    "--lr", str(rl_cfg["lr"]),
                    "--iterations", str(rl_cfg["num_iterations"]),
                    "--trajs-per-iter", str(rl_cfg["trajectories_per_iter"]),
                    "--gamma", str(rl_cfg["gamma"]),
                    "--eval-every", str(rl_cfg["eval_every"]),
                    "--eval-episodes", str(rl_cfg["eval_episodes"]),
                    "--max-steps", str(rl_cfg["max_steps"]),
                    "--seed", str(seed),
                    "--env", env_id,
