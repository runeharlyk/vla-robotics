...
def main(
    ...
    n_action_steps: int = typer.Option(
        1,
        "--n-action-steps",
        help="For LIBERO eval, execute this many actions from each sampled policy chunk before re-planning.",
    ),
    rollout_sampler: str = typer.Option(
        "normal",
        "--rollout-sampler",
        help="Action sampler for LIBERO rollout eval: normal or flow_sde.",
    ),
    flow_grpo_sigma: float = typer.Option(
        0.10,
        "--flow-grpo-sigma",
        help="Flow-SDE noise scale used when --rollout-sampler flow_sde.",
    ),
