...
uv run python language_diagnostics/libero_prompt_variant_run.py \
  --prompt-plan-path language_diagnostics/variant_prompt_plan_spatial_nr3.json \
  --episodes 50 \
  --device cuda \
  --progress-every 10 \
  --output-dir language_diagnostics/outputs/spatial/nr3/$LSB_JOBID
