...
DEFAULT_VARIANTS_JSON = Path("language_diagnostics/variant_prompt_plan_full.json")
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "data" / "h5" / "libero"
DEFAULT_ROLLOUTS_PER_TASK = 50
...
def _load_plan_tasks(path: Path) -> dict[str, list[int]]:
    payload = json.loads(path.read_text(encoding="utf-8"))
    by_suite: dict[str, list[int]] = defaultdict(list)
    for task in payload.get("tasks", []):
        suite = str(task.get("suite", "")).strip().lower()
        if suite not in LIBERO_SUITES:
            continue
        task_id = _as_int(task.get("task_id"), default=-1)
        if task_id < 0:
            continue
        by_suite[suite].append(task_id)

    if not by_suite:
        raise ValueError(f"No valid tasks found in {path}")

    return {suite: sorted(dict.fromkeys(task_ids)) for suite, task_ids in by_suite.items()}
...
def _select_task_rollouts(
    ...
    if rollouts_per_task <= 0:
        raise ValueError("rollouts_per_task must be > 0")

    chosen_episodes: list[int] = []
    for task_id in chosen_tasks:
        need = rollouts_per_task
        candidates = episodes_by_task[task_id]
        replace = len(candidates) < need
        if replace:
            print(
                f"warning: task {task_id} has only {len(candidates)} unique episodes; sampling with replacement to reach {need} rollouts"
            )
        picks = rng.choice(np.asarray(candidates), size=need, replace=replace)
...
def export_suite(
    ...
    out_path = output_dir / f"libero_{suite}_tasks{num_tasks}_rollouts{rollouts_per_task}.h5"
    if out_path.exists():
        out_path.unlink()

    episode_data: dict[int, dict[str, Any]] = {}
    for row in ds:
        ...
        episode_data[ep]["actions"].append(action)
        episode_data[ep]["states"].append(state)
        episode_data[ep]["cam0"].append(cam0)
        episode_data[ep]["cam1"].append(cam1)

    with h5py.File(out_path, "w") as f:
        f.attrs["suite"] = suite
        f.attrs["repo_id"] = repo_id
        f.attrs["num_tasks"] = num_tasks
        f.attrs["num_rollouts"] = rollouts_per_task
        f.attrs["selected_task_indices"] = np.asarray(selected_tasks, dtype=np.int32)

        demos_group = f.create_group("demonstrations")
        for demo_idx, ep in enumerate(selected_eps):
...
def main() -> None:
    ...
    parser.add_argument(
        "--suites",
        nargs="+",
        default=list(LIBERO_SUITES.keys()),
    ...
    parser.add_argument(
        "--variants-json",
        type=Path,
        default=DEFAULT_VARIANTS_JSON,
        help="Prompt plan JSON containing the full suite/task list",
    )
    parser.add_argument("--rollouts-per-task", type=int, default=DEFAULT_ROLLOUTS_PER_TASK, help="episodes per task")
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Explicit task indices to export instead of using the plan's task ids.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    plan_tasks = _load_plan_tasks(args.variants_json)
    suites = [suite for suite in args.suites if suite in plan_tasks]
    if not suites:
        raise ValueError(f"No requested suites found in {args.variants_json}")

    for suite in suites:
        task_indices = plan_tasks[suite] if args.task_indices is None else list(args.task_indices)
        if not task_indices:
            print(f"skipping {suite}: no tasks found in prompt plan")
            continue
        print(
            f"\nPreparing suite '{suite}' with {len(task_indices)} task(s) and {args.rollouts_per_task} rollout(s) per task"
        )
        path = export_suite(
            suite=suite,
